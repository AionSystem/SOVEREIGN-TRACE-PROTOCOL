...
SOVEREIGN-TRACE-STAMP-FROZEN-3.0-RETIRED, and create
FROZEN-4.0 from scratch with documented correction.
...
Valid date range:
  ...
  Input dates before 1582 will raise ValueError — pre-reform dates
...
FILE INTEGRITY
──────────────
_FROZEN_FILE_SHA256 is set at release time to SHA-256 of this file.
Operator deployment verification:
  sha256sum sovereign_trace_stamp.py
  Compare against _FROZEN_FILE_SHA256 below.
The bootstrap problem (a file cannot contain its own hash) means
this constant is set externally after the file is complete.
═══════════════════════════════════════════════════════════════════
"""
...
class SovereignStamp:
    ...
    def check_invariant(self) -> bool:
        """
        Verify internal consistency of this stamp.
        Returns True if all fields satisfy the SovereignStamp invariant.
        Does NOT verify the seal against an entry (entry is not stored).
        """
...
def _jd_from_gregorian(year: int, month: int, day: int) -> int:
    ...
    ValueError
        ...
        Dates before Gregorian reform require Julian calendar —
        the Gregorian formula produces incorrect JD values.
    """
...
def _month_lengths(h_year: int) -> tuple:
    ...
    ValueError
        ...
        This indicates an algorithm error — do not patch, create FROZEN-4.0.
    """
    ...
    if ylen not in _LEGAL_YEAR_LENGTHS:
        raise ValueError(
            ...
            "Algorithm error — do not patch. Create FROZEN-4.0 with correction."
        )
...
def _gregorian_to_hebrew(year: int, month: int, day: int) -> tuple:
    ...
    raise ValueError(
        ...
        "Algorithm error — do not patch. Create FROZEN-4.0."
    )
...
def stamp(entry_text: str, dt: datetime = None) -> SovereignStamp:
    ...
    ValueError
        ...
        If date is outside valid Gregorian range [1582, 4000].
        If calendar algorithm produces illegal year length.
        If Hebrew conversion overflows (algorithm error — create FROZEN-4.0).
    TypeError
...
def _run_self_test(verbose: bool = True) -> None:
    ...
    first principles and create FROZEN-4.0 if correction is needed.
    ...
    if verbose:
        print("═══ SOVEREIGN TRACE STAMP FROZEN-3.0 — SELF-TEST ═══")
    ...
    # Rosh Hashanah 5787 (start of a new leap year): Sep 22, 2026. [D] Hebcal verified.
    rh87_y, rh87_m, rh87_d = _gregorian_to_hebrew(2026, 9, 22)
    check("RH 5787 year",  rh87_y, 5787)
    check("RH 5787 month", rh87_m, "Tishri")
    check("RH 5787 day",   rh87_d, 1)

    # Day before RH 5787 = 29 Elul 5786. [D] Hebcal verified.
    erh87_y, erh87_m, erh87_d = _gregorian_to_hebrew(2026, 9, 21)
    ...
    check("Stamp carries FROZEN-3.0 version", ts_anchor.version, "FROZEN-3.0")
    ...
    # FROZEN-1.0 produced "15 Adar 5786" for this date. Neither 2.0 nor 3.0 must.
    ...
    if failures:
        ...
        raise AssertionError(
            ...
            "If correction is needed: create FROZEN-4.0."
        )
    else:
        if verbose:
